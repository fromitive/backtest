...
from backtest.use_cases.make_crypto_backtest import MarketType, make_crypto_selector_reference_makretcap
...
def sample_selector_reference_list():
    ...
    eth_result = {"date": ["2020-01-02 00:00:00", "2020-01-03 00:00:00"], "marketcap": [4.4, 5.5]}
...
def stockdata_list(dict_stock_data_list):
    stockdata1 = StockData.from_dict(dict_data=dict_stock_data_list[0], symbol="BTC")
    stockdata2 = StockData.from_dict(dict_data=dict_stock_data_list[1], symbol="ETH")
...
def test_make_crypto_selector_marketcap(coingeckorepo, get_bithumb_symbol, sample_selector_reference_list):
    ...
    to_date = "2020-01-03"
    coingeckorepo.side_effect = [sample_selector_reference_list[0], sample_selector_reference_list[1]]
    get_bithumb_symbol.return_value = ["BTC", "ETH"]
    response = make_crypto_selector_reference_makretcap(market=MarketType.BITHUMB, from_date=from_date, to_date=to_date)
