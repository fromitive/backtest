...
def dict_strategy_result():
    return {
        "name": [
            (StrategyResultColumnType.BUY, 1),  # final strategy column and weight score
...
def _inner_strategy_execute_result(dict_strategy_result):
    df = pd.DataFrame(dict_strategy_result, columns=["name", "date"])
...
def test_backtest_execute_without_options(
    strategy_execute_result, strategy_list, stockdata_list, _inner_strategy_execute_result
):
