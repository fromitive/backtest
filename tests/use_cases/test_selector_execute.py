...
def sample_selector_reference():
    dict = {"date": ["2022-10-11", "2022-11-11"], "dummy1": [1.1, 2.2], "dummy2": ["11", "22"]}
...
def sample_selector_result():
    dict = {
        "SYMBOL1": [(SelectorResultColumnType.KEEP, 100), (SelectorResultColumnType.SELECT, 100)],
        "SYMBOL2": [(SelectorResultColumnType.KEEP, 100), (SelectorResultColumnType.SELECT, 100)],
        "date": ["2022-10-30", "2022-11-11"],
    }
    df = pd.DataFrame(dict, columns=["SYMBOL1", "SYMBOL2", "date"])
...
def test_selector_execute(sample_selector_reference, sample_selector_result):
    ...
    selector = Selector(
        name="test selector",
        weight=100,
        selector_function=test_function,
        options={"param1": "value1", "param2": "value2"},
        reference=sample_selector_reference,
    )
