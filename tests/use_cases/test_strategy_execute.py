...
from backtest.use_cases.strategy_execute import basic_function, strategy_execute
...
def stockdata_list(dict_stock_data_list):
    stockdata1 = StockData.from_dict(dict_data=dict_stock_data_list[0], symbol="teststock1")
    stockdata2 = StockData.from_dict(dict_data=dict_stock_data_list[1], symbol="teststock2")
    stockdata3 = StockData.from_dict(dict_data=dict_stock_data_list[2], symbol="teststock3")
...
def dict_strategy_result():
    return {
        "test strategy": [
            (StrategyResultColumnType.BUY, 1),  # type and strategy weight
...
def strategy_result_data_frame(dict_strategy_result):
    df = pd.DataFrame(dict_strategy_result, columns=["test strategy", "date"])
...
def sample_strategy_result(dict_strategy_result):
    strategy_result = StrategyResult.from_dict(dict_strategy_result, target="TEST")
...
def test_strategy_execute(sample_stock_data, strategy_result_data_frame):
    ...
    response = strategy_execute(strategy_list=[strategy], stockdata=sample_stock_data)
...
def test_strategy_basic_function(stockdata_list):
    strategy = Strategy(name="test strategy", weight=1999, target="TARGET", function=basic_function)
    for stockdata in stockdata_list:
        response = strategy_execute(strategy_list=[strategy], stockdata=stockdata)
        strategy_result = response.value
        assert isinstance(response, ResponseSuccess)
        assert isinstance(strategy_result, StrategyResult)
