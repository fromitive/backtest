...
def test_build_stock_data_from_repo_request_accepted_filters(interval, payment):
    filters = {
        "order__eq": "BTC",
        "payment__eq": payment,
        "chart_interval__eq": interval,
        "from__eq": "1990-01-01",
        "to__eq": "1990-01-01",
        "start_time__eq": "06:00",
        "end_time__eq": "23:30",
    }
...
def test_build_stock_data_from_repo_request_rejected_filters_with_wrong_payment(payment):
    filters = {"order__eq": "BTC", "payment__eq": payment, "chart_interval__eq": "24h"}
    request = build_stock_data_from_repo_request(filters=filters)
    assert request.has_errors()
    assert request.errors[0]["parameter"] == "filters"
    assert bool(request) is False
...
def test_build_stock_data_from_repo_request_rejected_filters_with_wrong_chart_interval(chart_interval):
    filters = {"order__eq": "BTC", "payment__eq": ["KRW"], "chart_interval__eq": chart_interval}
    request = build_stock_data_from_repo_request(filters=filters)
    assert request.has_errors()
    assert request.errors[0]["parameter"] == "filters"
    assert bool(request) is False
...
def test_build_stock_data_from_repo_request_rejected_filter_with_invalied_date(invalid_date):
    filters = {"chart_interval__eq": "24h", "from__eq": invalid_date, "to__eq": invalid_date}
    request = build_stock_data_from_repo_request(filters=filters)
    assert request.has_errors()
    assert request.errors[0]["parameter"] == "filters"
...
def test_build_stock_data_from_repo_request_accepted_filter_with_valied_future_date(future_date):
    filters = {"chart_interval__eq": "24h", "to__eq": future_date}
    request = build_stock_data_from_repo_request(filters=filters)
    assert request.filters == filters
    assert bool(request) is True
...
def test_build_stock_data_from_repo_request_rejected_filter_with_valied_future_date(future_date):
    filters = {"chart_interval__eq": "24h", "from__eq": future_date}
    request = build_stock_data_from_repo_request(filters=filters)
    assert request.has_errors()
    assert request.errors[0]["parameter"] == "filters"
    for error in request.errors:
