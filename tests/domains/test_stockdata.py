...
def test_init_stockdate_with_parameters():
    sto = StockData(
        symbol="test", data=pd.DataFrame(columns=["open", "high", "low", "close", "volume"], index=DatetimeIndex([]))
    )
    assert sto.symbol == "test"
    assert list(sto.data.columns) == ["open", "high", "low", "close", "volume"]
    # assert isinstance(sto.data.index, DatetimeIndex)
...
def test_init_stockdata_from_csv_file(read_csv):
    read_csv.return_value = pd.DataFrame(
        columns=["date", "open", "high", "low", "close", "volume"], index=DatetimeIndex([])
    )
