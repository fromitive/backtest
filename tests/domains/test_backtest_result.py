...
def dict_backtest_result():
    return {
        "stock_bucket": [
            ...
            [("BTC", "2022-10-30", -1.0), ("ETH", "2022-10-30", 0.0), ("ETH", "2022-10-31", 0.0)],
            [("XRP", "2022-11-01", 0.5)],
            [("TEST", "2022-12-05", 10.0)],
        ],
        "date": ["2022-10-30", 1388070000000, "2022-02-09", "2022-04-07"],
        "total_profit": [1.0, 1.0, 1.1, -2.1],
    }


@pytest.fixture(scope="function")
def dict_backtest_result_dataframe(dict_backtest_result):
    df = pd.DataFrame(dict_backtest_result, columns=["stock_bucket", "total_profit", "date"])
