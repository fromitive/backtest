...
def _search_coingecko_symbol_id(symbol):
    try:
        coin_csv = pd.read_csv(os.path.join(CSV_REPO_PATH, GECKO_COIN_LIST_CSV_PATH))
...
def get_coingecko_symbol_id(symbol: str) -> int:
    ...
    if isinstance(df, pd.DataFrame):
        ...
        try:
            ...
            print("symbol value({symbol}) is not found execute search.. ".format(symbol=symbol))
            symbol_id = _search_coingecko_symbol_id(symbol)

            df = pd.concat([df, pd.DataFrame([{"symbol": symbol, "id": symbol_id}])])
            df.to_csv(os.path.join(CSV_REPO_PATH, GECKO_CSV_PATH), index=False)
            return symbol_id
