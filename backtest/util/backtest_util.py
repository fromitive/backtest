...
def grid_search_optimize_one_strategy_parameter(
    strategy_function,
    target_stockdata_list: List[StockData],
    param_grid: dict,
    inverse: bool = False,
    verbose: bool = False,
) -> pd.DataFrame:
    ...
    for idx, param_combination in enumerate(param_combinations_list, start=1):
        ...
        for stock_idx, stockdata in enumerate(target_stockdata_list, start=1):
            if verbose:
                print(
                    "\rtest[{idx} / {total_test_len}] progress {stock_idx} / {total} ".format(
                        idx=idx, total_test_len=combinations_len, stock_idx=stock_idx, total=stock_len
                    ),
                    end="",
                    flush=True,
                )
            strategy = Strategy(
                name=strategy_function.__name__,
                function=strategy_function,
                weight=1,
                inverse=inverse,
                options=param_combination,
            )
            ...
            if isinstance(response, ResponseSuccess):
                ...
                total_profit = backtest_df["total_profit"].expanding().sum().iloc[-1]
                potential_min_profit = backtest_df["total_potential_profit"].min()
                total_profit_list.append((total_profit, stockdata.symbol))
                total_min_potential_profit_list.append((potential_min_profit, stockdata.symbol))
        mean_total_profit = statistics.mean([value for value, symbol in total_profit_list])
        mean_min_potential_profit = statistics.mean([value for value, symbol in total_min_potential_profit_list])
        ...
        if verbose:
            print(
                "test[{idx}]: {function_name} parmater_combination : {param_combination} inverse : {inverse} mean_total_profit : {mean_total_profit} mean_min_potential_profit: {mean_min_potential_profit} max_total_profit[{max_total_symbol}]: {max_total_profit}, min_potential_profit[{min_potential_symbol}]: {min_potential_profit}".format(
                    idx=idx,
                    function_name=strategy_function.__name__,
                    param_combination=param_combination,
                    inverse=inverse,
                    mean_total_profit=mean_total_profit,
                    mean_min_potential_profit=mean_min_potential_profit,
                    max_total_symbol=max_total_symbol,
                    max_total_profit=max_total_profit,
                    min_potential_symbol=min_potential_symbol,
                    min_potential_profit=min_potential_profit,
                )
            )
        for k, v in param_combination.items():
            result_dict[k].append(v)
        result_dict["mean_total_profit"].append(mean_total_profit)
