...
class BinanceRepo:
    ...
    def get(self, filters=None):
        if filters:
            ...
            self.order_currency = filters["order__eq"].upper() if "order__eq" in filter else "BTC"
            self.payment_currency = filters["payment__eq"].upper() if "payment__eq" in filter else "USDT"
            self.chart_intervals = filters["chart_interval__eq"] if "chart_interval__eq" in filter else "1d"
            if self.chart_intervals == "24h":
                self.chart_intervals = "1d"
            self.from_date = datetime.strptime(filters["from__eq"], "%Y-%m-%d") if "from__eq" in filter else ""
            self.to_date = datetime.strptime(filters["to__eq"], "%Y-%m-%d") if "to__eq" in filter else ""
        ...
        request_params["interval"] = request_params["interval"].format(chart_intervals=self.chart_intervals)
        ...
        before_date = ""
        while True:
            response = requests.get(self.API_URL, headers=self.API_HEADERS, params=request_params)
            if response.status_code == 200:
                result_list = response.json()
                if result_list == []:
                    break
                data_last_date = datetime.fromtimestamp(int(result_list[0][0]) / 1000).replace(
            ...
            if self.from_date == "":
                ...
                for idx, candledata in enumerate(result_list):
                    compare_date = datetime.fromtimestamp(int(candledata[0]) / 1000).replace(
                        hour=0, minute=0, second=0, microsecond=0
                    )
                ...
                raise Exception("date_convert error data_last_date: ", data_last_date)
            temp_list += result_list
            time.sleep(0.3)
        columns = [
            "open_time",
