...
class CoinGeckoRepo:
    ...
    def get(self, filters=None):
        if filters:
            ...
            self.to_date = filters["to__eq"] if "to__eq" in filter else datetime.strftime(datetime.now(), "%Y-%m-%d")
            self.from_timestamp = convert_timestamp(self.from_date)
            self.to_timestamp = convert_timestamp(self.to_date) + 86320  # for add 23:59
            self.symbol_id = get_coingecko_symbol_id(self.symbol)
        request_url = self.API_URL.format(
            symbol_id=self.symbol_id, from_timestamp=self.from_timestamp, to_timestamp=self.to_timestamp
        )
        response = requests.get(request_url, headers=self.API_HEADERS)

        if response.status_code == 200:
            dict_data = response.json().get("stats")
            temp_df = pd.DataFrame(dict_data, columns=["time", "marketcap"])
