...
class UpbitRepo:
    ...
    def get(self, filters=None):
        if filters:
            ...
            self.to_date = datetime.strptime(filters["to__eq"], "%Y-%m-%d") if "to__eq" in filter else datetime.now()
            ...
            if unit == "m":
                if interval_value not in [1, 3, 5, 15, 10, 30, 60, 240]:
                    ...
                    self.chart_interval_kind = "{}/{}".format("minutes", interval_value)
                    self.unit = "M"
            elif unit == "d":
        ...
        while True:
            ...
            if response.status_code == 200:
                ...
                if result_list == []:
                    break
                data_last_date = datetime.strptime(result_list[-1]["candle_date_time_utc"], "%Y-%m-%dT%H:%M:%S")
                if self.chart_intervals:
                    data_last_date = data_last_date.replace(hour=0, minute=0, second=0, microsecond=0)
            ...
            if self.from_date == "":
                ...
                while result_list[0] != result_list[idx]:
                    compare_date = datetime.strptime(result_list[idx]["candle_date_time_utc"], "%Y-%m-%dT%H:%M:%S")
