...
from backtest.use_cases.strategy_execute import _basic_weight_score_function, strategy_execute
...
def _init_backtest_result(column_name_and_type: dict, base_index: pd.Index):
    backtest_result_raw = pd.DataFrame(index=base_index, columns=list(column_name_and_type.keys()))
...
def backtest_execute(
    backtest: Backtest,
    init_invest_money: float = 10000000.0,
    invest_rate: float = 0.01,
    minimum_buy_count: float = 5.0,
    verbose: bool = False,
    save_strategy_result: bool = False,
    save_raw_csv_file: str = "",
    ...
    stockdata_dict = {stockdata.symbol: stockdata for stockdata in backtest.stockdata_list}
    stock_bucket_dict = {
        stockdata.symbol: pd.DataFrame(columns=["bucket", "invest_money"], index=base_index).fillna(0)
    ...
    backtest_result_raw = _init_backtest_result(column_name_and_type, base_index)
    ...
    for num, index in enumerate(base_index, start=1):
        ...
        for stockdata in pick_stockdata_list:
            ...
            if not isinstance(strategy_result_dict[stockdata.symbol], StrategyResult):
                response = strategy_execute(
                    ...
                    strategy_list=pre_strategy_list,
                    stockdata=stockdata,
                    save_strategy_result=save_strategy_result,
            ...
            strategy_result_of_day, weight_score = strategy_result_dict[stockdata.symbol].value[index]
            ...
            if weight_score < minimum_buy_count and strategy_result_of_day == StrategyResultColumnType.BUY:
                strategy_result_dict[stockdata.symbol].value[index] = (StrategyResultColumnType.KEEP, weight_score)
            ...
            if strategy_result_of_day == StrategyResultColumnType.BUY:
                ...
                if buy_total_money <= current_invest_money:
                    ...
                    if stockdata.symbol not in stock_temp_dict:
                        ...
                        stock_temp_dict[stockdata.symbol] += buy_score
                    stock_bucket_dict[stockdata.symbol].at[index, "bucket"] += buy_score
                    stock_bucket_dict[stockdata.symbol].at[index, "invest_money"] = buy_money
                    # buy stock end
        ...
        for symbol in stock_temp_dict:
            ...
            buy_list.sort(key=lambda profit_index: stockdata_dict[symbol].data.at[profit_index, "close"], reverse=True)
            for profit_index in buy_list:
                bucket_cnt = stock_bucket_df.at[profit_index, "bucket"]
                profit_money = stock_bucket_df.at[profit_index, "invest_money"] * (
            ...
            if strategy_result_of_day == StrategyResultColumnType.SELL:
                ...
                sell_cnt = math.ceil((weight_score / 100) * total_stock_length)
        ...
        backtest_result_raw.at[index, "total_potential_earn"] = total_potential_earn + current_invest_money
        backtest_result_raw.at[index, "current_money"] = current_invest_money
        ...
        if index == base_index[-2]:
            if verbose:
                ...
                for today_symbol in pick_stockdata_list:
                    print(
                        "{symbol_name} : {strategy_result}".format(
                            symbol_name=today_symbol.symbol,
                            strategy_result=strategy_result_dict[today_symbol.symbol].value[index],
                        )
                    )
    ...
    backtest_result_raw["total_potential_profit"] = (
        backtest_result_raw["total_potential_earn"] - init_invest_money
    ) / init_invest_money
