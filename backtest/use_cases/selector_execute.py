...
def basic_function(symbol_list: List[str], weight: int, name: str, reference: SelectorReference):
    response = SelectorResult(value=pd.DataFrame(index=reference.data.index, columns=[symbol_list]))
    for symbol in symbol_list:
        response.value[symbol] = [(SelectorResultColumnType.KEEP, weight)] * len(reference.data)
    return response


def rank_marketcap_function(
    ...
    def sample_function(r: pd.Series, symbol_list):
        target_list = list(r.nlargest(nlargest_number).nsmallest(select_pick).index)
        return [
            (SelectorResultColumnType.SELECT, weight)
            if col_name in target_list
            else (SelectorResultColumnType.KEEP, weight)
            for col_name in symbol_list
        ]

    rank_result = rank_df.apply(lambda r: sample_function(r, symbol_list=rank_df.columns), axis=1)
...
def generate_empty_selector_result_data(symbol_list: List[str], from_date: str = "1990-01-01", to_date: str = ""):
    ...
    df = pd.DataFrame(columns=symbol_list, index=date_series)
    df = df.fillna("dummy")
    for column in df.columns:
        df[column] = df[column].apply(
            lambda cell: {SelectorResultColumnType.KEEP: 0, SelectorResultColumnType.SELECT: 0}
        )
    return df
...
def selector_execute(selector_list: List[Selector], symbol_list: List, from_date="1990-01-01", to_date=""):
    ...
    selector_total_result_df = generate_empty_selector_result_data(symbol_list, from_date=from_date, to_date=to_date)
    ...
    try:
        for selector in selector_list:
            ...
            if isinstance(response, ResponseFailure):
                ...
                selector_result_df = selector_result_df.reindex(selector_total_result_df.index)
        ...
        selector_total_result_df = selector_total_result_df.apply(lambda row: _sum_total_selector_result(row), axis=1)
