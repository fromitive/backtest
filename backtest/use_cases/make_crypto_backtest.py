...
from backtest.use_cases.standardize_selector_reference import standardize_selector_reference
...
def get_upbit_symbol():
    ...
    symbol_list = [symbol["market"][4:] for symbol in res if symbol["market"].split("-")[0] == "KRW"]
    return symbol_list


def make_crypto_selector_reference_makretcap(
    market: MarketType, from_date: str = "", to_date: str = "", cache: bool = False
    ...
    if market == MarketType.BITHUMB:
        ...
        return ResponseFailure(
            type_=ResponseTypes.SYSTEM_ERROR, message="market not supported value = {}".format(market)
        )
    ...
    if cache:
        try:
            selector_reference = SelectorReference.from_csv(CSV_PATH, symbol=str_symbol)
    ...
    for idx, symbol in enumerate(symbol_list, start=1):
        print(
            "make selector reference {symbol} {idx}/{len_symbol_list}".format(
        ...
        response = selector_reference_from_repo(CoinGeckoRepo(), request=request, cache=cache)
        if isinstance(response, ResponseFailure):
            return ResponseFailure(
                type_=ResponseTypes.SYSTEM_ERROR,
                message="symbol : {} fail to load from coingecko repository".format(symbol),
            )
        else:  # ResponseSuccess
            selector_reference_list.append(response.value)
    standardize_selector_reference(selector_reference_list)
    selector_reference_data = pd.DataFrame(index=selector_reference_list[0].data.index, columns=symbol_list)
    ...
    total_selector_reference = SelectorReference(symbol=str_symbol, data=selector_reference_data)
    if cache:
        total_selector_reference.to_csv(CSV_PATH)
    return ResponseSuccess(total_selector_reference)


def make_crypto_stockdata_list(
    market: MarketType, selector_result: SelectorResult, cache: bool = False
) -> List[StockData] | ResponseFailure:
    ...
    if market == MarketType.BITHUMB:
        ...
        return ResponseFailure(
            type_=ResponseTypes.SYSTEM_ERROR, message="market repo not supported value = {}".format(market)
        )
    ...
    for idx, symbol in enumerate(stockdata_symbol_bucket, start=1):
        print(
            "Get StockData[{symbol}] From {repo_name} {current}/{total}".format(
        ...
        response = stockdata_from_repo(stockdata_repo(), request=request, cache=cache)
        if isinstance(response, ResponseFailure):
            return ResponseFailure(
                ResponseTypes.SYSTEM_ERROR,
                "failed to load repository symbol value : {symbol} in {repo_name}".format(
                    symbol=symbol, repo_name=stockdata_repo
                ),
            )
        else:
