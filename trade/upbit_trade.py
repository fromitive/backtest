...
class UpbitTrade(Trade):
    ...
    def trade(self, symbol: str, units: str, type: str, price: float, payment_currency: str = 'KRW') -> str:
        order_id = ''
        if type == 'BUY':
            order_id = self.upbit.buy_limit_order(ticker="{payment_currency}-{symbol}".format(payment_currency=payment_currency, symbol=symbol), price=price, volume=units)
        elif type == 'SELL':
            order_id = self.upbit.sell_limit_order(ticker="{payment_currency}-{symbol}".format(payment_currency=payment_currency, symbol=symbol), price=price, volume=units)
