...
from backtest.use_cases.strategy_execute import ema_local_min_max_trandline_function,ema_fibonacci,candle_stick_pattern,ema_swing_low_function
    ...
    while try_count < GLOBAL_MAX_TRY_COUNT:
        ...
        if order_id and is_trade:
            ...
            if not order_status:
                ...
                if types == 'BUY':
                    buy_dict = {'flag': False, 'symbol': '', 'order_id': '', 'sell_profit': 0.025, 'sell_loss': -0.015, 'default_sell_loss': -0.015, 'default_sell_profit': 0.025}
            ...
            try:
                ...
                if GLOBAL_VERBOSE:
                    ...
                    for symbol in symbols:
                        tradeable, extra_data, trade_strategy_package = is_tradeable_check(symbol, verbose=GLOBAL_VERBOSE, trade_strategy_list=trade_strategy_list, delay=10)
                        if tradeable:
                            if GLOBAL_VERBOSE:
...
GLOBAL_BUY_DICT = {'flag': False, 'symbol': '', 'order_id': '', 'sell_profit': default_sell_profit, 'sell_loss': default_sell_loss, 'default_sell_loss': default_sell_loss, 'default_sell_profit': default_sell_profit}
GLOBAL_VERBOSE = True
