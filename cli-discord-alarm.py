...
def build_stockdata(symbol: str, from_date: str, cache: bool = False, big_stock: bool = False):
    request = build_stock_data_from_repo_request(filters={"order__eq": symbol, "from__eq": from_date})
...
def discord_fib_message(symbol, current_time, discord_type, strategy_name, **kwargs):
    with open(discord_type, "r") as f:
        ...
        content = "[{current_time}] [{strategy_name}] BUY *{symbol}*\n\nartifects\n{artifacts}".format(
            current_time=current_time, strategy_name=strategy_name, symbol=symbol, artifacts=artifacts
        )
        ...
        try:
            ...
            if market_timing_trade_strategy_list:
                ...
                for symbol in symbols:
                    tradeable, extra_data, trade_strategy_package = is_tradeable(
                        symbol=symbol, verbose=GLOBAL_VERBOSE, trade_strategy_list=market_timing_trade_strategy_list
                    )
                    if tradeable and symbol not in sent_symbol_set:
                        trade_routine = threading.Thread(
                            target=discord_message,
                            args=(symbol, current_time, "discord_url.txt", trade_strategy_package),
                    ...
                    if hour in [5, 9, 18] and minute == 0 and symbol not in sent_symbol_set:
                        print("[DEBUG] current symbol set {}".format(sent_symbol_set))
                        trade_routine = threading.Thread(
                            target=discord_message,
                            args=(symbol, current_time, "discord_check.txt", trade_strategy_package),
                            kwargs={"extra_data": extra_data},
                        )
                        trade_routine.start()
...
package2 = TradeStrategyPackage(
    strategy_function=swing_search,
